...
import { Building2, MapPin, Save, XCircle, Loader2, MessageSquare, Image as ImageIcon, Zap, Timer, Bell, Table, Trash2, Plus, ChevronDown } from 'lucide-react';
...
export default function UmumPage() {
  ...
  // Expandable panel state for sub-sections under Pusat Kontrol
    ...
    if (mosque) {
      ...
      if (mosque.settings) {
        ...
        if (settings.slideInterval !== undefined) {
          setSlideInterval(Math.floor((settings.slideInterval ?? 30000) / 1000));
        } else {
          setSlideInterval(30);
        }
  ...
  const handleMapChange = (newLat: number, newLng: number) => {
    setFormState(prev => ({ ...prev, lat: String(newLat), long: String(newLng) }));
  };
  ...
  const handleGetGeoLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        setFormState(prev => ({ ...prev, lat: String(position.coords.latitude), long: String(position.coords.longitude) }));
        setGeoLoading(false);
  ...
  const saveLatLong = async () => {
    if (!formState.lat || !formState.long) { showToast('Latitude dan Longitude harus diisi'); return; }
    const latNum = Number(formState.lat);
    const longNum = Number(formState.long);
    if (isNaN(latNum) || latNum < -90 || latNum > 90) { showToast('Latitude harus antara -90 dan 90'); return; }
    if (isNaN(longNum) || longNum < -180 || longNum > 180) { showToast('Longitude harus antara -180 dan 180'); return; }
    setSaving(true);
    try {
      const res = await fetch('/api/mosque/update', {
        method: 'PUT', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${user?.token}` },
        body: JSON.stringify({ lat: latNum, long: longNum, calculation_method: formState.calculationMethod }),
      });
  ...
  const saveRunningText = async () => {
    ...
    try {
      const res = await fetch('/api/mosque/update', {
        method: 'PUT', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${user?.token}` },
        body: JSON.stringify({ runningText1: formState.runningText1, runningText2: formState.runningText2 }),
      });
      if (!res.ok) { const data = await res.json(); showToast(data.error || 'Gagal menyimpan'); return; }
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return;
    if (file.size > 10 * 1024 * 1024) { showToast('File terlalu besar (max 10MB)'); return; }
    ...
    return new Promise((resolve) => {
      ...
      reader.onload = (event) => {
        ...
        img.onload = () => {
          ...
          if (height > 1080) { width = Math.round((width * 1080) / height); height = 1080; }
  ...
  const handleSliderFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files.length > remainingCount) { showToast(`Maksimal 10 gambar. Tersisa ${remainingCount} slot.`); return; }
    const compressedImages: string[] = [];
    for (let i = 0; i < files.length; i++) {
      const file = files[i];
      if (file.size > 10 * 1024 * 1024) { showToast(`File ${file.name} terlalu besar`); return; }
      try { compressedImages.push(await compressImage(file)); }
      catch (error) { console.error('Error compressing image:', error); showToast('Gagal memproses gambar'); return; }
    }
  ...
  const saveBackgroundSlider = async () => {
    if (sliderBackgrounds.length === 0) { showToast('Tambahkan minimal 1 gambar'); return; }
    setSaving(true);
    try {
      const res = await fetch('/api/mosque/upload-backgrounds', {
        method: 'PUT', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${user?.token}` },
        body: JSON.stringify({ backgrounds: sliderBackgrounds, slideInterval: slideInterval * 1000, animationType: animationType, isInfiniteLoop: isInfiniteLoop }),
      });
      if (!res.ok) { const data = await res.json(); showToast(data.error || 'Gagal menyimpan background slider'); }
      else { showToast('Background Slider berhasil disimpan & dikirim ke TV'); queryClient.invalidateQueries({ queryKey: ['mosque'] }); }
    } catch (error) { showToast('Gagal menyimpan'); }
    setSaving(false);
  };

  const saveBackground = async () => {
    if (!backgroundPreview) { showToast('Pilih gambar terlebih dahulu'); return; }
    setSaving(true);
    try {
      ...
      if (!res.ok) { const data = await res.json(); showToast(data.error || 'Gagal menyimpan background'); }
      else { showToast('Background berhasil disimpan & dikirim ke TV'); queryClient.invalidateQueries({ queryKey: ['mosque'] }); }
    } catch { showToast('Gagal menyimpan'); }
    setSaving(false);
  };
  ...
  const saveTimerSettings = async () => {
    if (!formState.isMuadzin) { showToast('Mode Muadzin harus diaktifkan terlebih dahulu untuk menggunakan timer'); return; }
    setSaving(true);
    try {
      const res = await fetch('/api/mosque/upload-timer', {
        method: 'PUT', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${user?.token}` },
        body: JSON.stringify({ timerSettings: { pre_adhan_countdown_minutes: timerPreAdhanMinutes, iqamah_duration_minutes: { subuh: timerIqamahSubuh, dzuhur: timerIqamahDzuhur, ashar: timerIqamahAshar, maghrib: timerIqamahMaghrib, isya: timerIqamahIsya }, beep_sound_enabled: timerBeepEnabled, beep_count: timerBeepCount, show_silent_icon_during_prayer: showSilentIcon, adhan_display_duration: adhanDisplayDuration } }),
      });
      if (!res.ok) { const data = await res.json(); showToast(data.error || 'Gagal menyimpan timer'); }
      else { showToast('Timer berhasil disimpan & dikirim ke TV'); queryClient.invalidateQueries({ queryKey: ['mosque'] }); }
    } catch (error) { showToast('Gagal menyimpan'); }
    setSaving(false);
  };

  const saveAnnouncements = async () => {
    if (!formState.isMuadzin) { showToast('Mode Muadzin harus diaktifkan terlebih dahulu untuk menggunakan pengumuman'); return; }
    setSaving(true);
    try {
      const res = await fetch('/api/mosque/upload-timer', {
        method: 'PUT', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${user?.token}` },
      ...
      if (!res.ok) { const data = await res.json(); showToast(data.error || 'Gagal menyimpan pengumuman'); }
      else { showToast('Pengumuman berhasil disimpan & dikirim ke TV'); queryClient.invalidateQueries({ queryKey: ['mosque'] }); }
    } catch (error) { showToast('Gagal menyimpan'); }
    setSaving(false);
  };

  const addAnnouncement = () => {
    if (!newAnnouncement.title) { showToast('Judul pengumuman harus diisi'); return; }
    const newAnn: Announcement = { id: Date.now().toString(), type: activeAnnouncementType, ...newAnnouncement };
  ...
  const removeTableHeader = (index: number) => {
    if (tableHeaders.length <= 1) { showToast('Minimal harus ada 1 kolom'); return; }
  ...
  const removeTableRow = (index: number) => {
    if (tableRows.length <= 1) { showToast('Minimal harus ada 1 baris'); return; }
    setTableRows(prev => prev.filter((_, i) => i !== index));
  };

  const saveTableData = async () => {
    if (!formState.isMuadzin) { showToast('Mode Muadzin harus diaktifkan terlebih dahulu untuk menggunakan tabel'); return; }
    setSaving(true);
    try {
  ...
  if (!user) return null;
  if (user.role === 'superadmin') { void router.push('/admin/masjid'); return null; }

  const hasCoords = formState.lat && formState.long;
  const hasBackground = backgroundPreview || formState.background;

  // Toggle section helper for collapsible panels in Pusat Kontrol
  ...
  return (
    ...
    <style jsx global>{`
      ...
      .leaflet-marker-pane, .leaflet-icon-pane, .leaflet-overlay-pane { z-index: 11 !important; }
      .leaflet-interactive { z-index: 12 !important; }
      .leaflet-popup-pane { z-index: 15 !important; }
      .leaflet-tooltip-pane { z-index: 16 !important; }
    `}</style>
    <div className="max-w-4xl mx-auto" style={{ animation: 'fadeSlideUp 0.3s ease-out' }}>
      ...
      <div className="space-y-3">
       ...
       {/* INFO ACCORDION */}
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input value={formState.lat} onChange={e => setFormState(prev => ({ ...prev, lat: e.target.value }))} className="input mt-1 bg-slate-800/50 border-slate-700 text-white font-mono" placeholder="-6.200000" type="text" />
              </div>
              <div>
                <label className="text-sm font-medium text-foreground">Longitude</label>
                <div className="flex gap-2 mt-1">
                  <input value={formState.long} onChange={e => setFormState(prev => ({ ...prev, long: e.target.value }))} className="input flex-1 bg-slate-800/50 border-slate-700 text-white font-mono" placeholder="106.816667" type="text" />
                  <button onClick={handleGetGeoLocation} disabled={geoLoading} className="shrink-0 w-10 h-10 flex items-center justify-center bg-yellow-500 hover:bg-yellow-600 text-white rounded-lg transition-colors disabled:opacity-50" title="Deteksi lokasi dari GPS browser">
                    {geoLoading ? <Loader2 className="w-5 h-5 animate-spin text-white" /> : <MapPin className="w-5 h-5" />}
                  </button>
                </div>
              </div>
            </div>
            ...
            {hasCoords ? (
              <div className={`relative rounded-lg overflow-hidden ${mapFullscreen ? 'fixed inset-0 z-[9999] bg-slate-900' : ''}`}>
                <div style={{ height: mapFullscreen ? '100vh' : '256px' }}>
                  <MapPicker key={`${formState.lat}-${formState.long}`} lat={parseFloat(formState.lat)} lng={parseFloat(formState.long)} onChange={handleMapChange} fullscreen={mapFullscreen} onCloseFullscreen={() => setMapFullscreen(false)} />
                </div>
                {!mapFullscreen && (
                  <button onClick={() => setMapFullscreen(true)} className="absolute bottom-2 right-3 p-2 bg-emerald-500 hover:bg-emerald-600 text-white rounded-lg transition-colors shadow-lg" style={{ zIndex: 1000 }} title="Buka peta fullscreen">
            ...
            <div>
              ...
              <select value={formState.calculationMethod} onChange={e => setFormState(prev => ({ ...prev, calculationMethod: e.target.value }))} className="input mt-1 bg-slate-800/50 border-slate-700 text-white w-full">
                {CALCULATION_METHODS.map(m => (<option key={m.value} value={m.value}>{m.label}</option>))}
              </select>
            </div>

            <button onClick={saveLatLong} disabled={saving} className="w-full flex items-center justify-center gap-2 bg-emerald-500 hover:bg-emerald-600 text-white shadow-lg shadow-emerald-500/30 px-4 py-2.5 text-sm font-semibold rounded-lg transition-colors disabled:opacity-50">
              {saving ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />}
        ...
        <AccordionItem title="Running Text" icon={<MessageSquare className="w-5 h-5" />}>
          <div className="space-y-5">
            <div>
              ...
              <textarea value={formState.runningText1} onChange={e => setFormState(prev => ({ ...prev, runningText1: e.target.value }))} className="input mt-1 bg-slate-800/50 border-slate-700 text-white w-full min-h-[80px] resize-y" placeholder="Contoh: SALDO KAS MASJID HARI JUMAT SEBESAR Rp. 80.345.609 TERIMAKASIH" />
            </div>
            <div>
              ...
              <textarea value={formState.runningText2} onChange={e => setFormState(prev => ({ ...prev, runningText2: e.target.value }))} className="input mt-1 bg-slate-800/50 border-slate-700 text-white w-full min-h-[80px] resize-y" placeholder="Contoh: BARANGSIAPA YANG BERSHOLAWAT KEPADAKU SEKALI, MAKA ALLAH AKAN BERSHOLAWAT KEPADANYA SEPULUH KALI" />
            </div>
            <button onClick={saveRunningText} disabled={saving} className="w-full flex items-center justify-center gap-2 bg-emerald-500 hover:bg-emerald-600 text-white shadow-lg shadow-emerald-500/30 px-4 py-2.5 text-sm font-semibold rounded-lg transition-colors disabled:opacity-50">
              {saving ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />}
              Simpan & Update TV
            </button>
          </div>
        ...
        <AccordionItem title="Background TV" icon={<ImageIcon className="w-5 h-5" />}>
          <div className="space-y-6">
            <div>
              ...
              <div onClick={() => fileInputRef.current?.click()} className="border-2 border-dashed border-slate-600 rounded-xl p-8 text-center cursor-pointer hover:border-emerald-500 hover:bg-emerald-500/5 transition-all">
            ...
            {hasBackground && (
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={() => { setBackgroundPreview(null); setFormState(prev => ({ ...prev, background: '' })); }} className="text-xs text-red-400 hover:text-red-300 transition-colors">Hapus</button>
                </div>
            ...
            <button onClick={saveBackground} disabled={saving || !hasBackground} className="w-full flex items-center justify-center gap-2 bg-emerald-500 hover:bg-emerald-600 text-white shadow-lg shadow-emerald-500/30 px-4 py-3 text-sm font-semibold rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
              {saving ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />}
              Simpan & Update TV
            </button>
          </div>
        ...
        <AccordionItem title="Background Slider TV" icon={<ImageIcon className="w-5 h-5" />}>
          <div className="space-y-6">
            <div>
              <label className="text-sm font-medium text-foreground">Upload Gambar</label>
              <p className="text-muted-foreground text-xs mb-3">Upload minimal 1 gambar untuk slideshow (max 10 gambar). Setiap gambar max 10MB.</p>
              <input type="file" accept="image/*" multiple ref={sliderFileInputRef} onChange={handleSliderFileSelect} className="hidden" />
              <div onClick={() => sliderFileInputRef.current?.click()} className="border-2 border-dashed border-slate-600 rounded-xl p-6 text-center cursor-pointer hover:border-emerald-500 hover:bg-emerald-500/5 transition-all">
            ...
            {sliderBackgrounds.length > 0 && (
              <div className="space-y-3">
                <label className="text-sm font-medium text-foreground">Urutan Slide ({sliderBackgrounds.length} gambar)</label>
                  ...
                  {sliderBackgrounds.map((bg, index) => (
                    <div key={index} className="flex items-center gap-3 p-2 rounded-lg bg-slate-800/50 border border-slate-700">
                      ...
                      <div className="flex gap-1">
                        <button onClick={() => moveSliderImage(index, -1)} disabled={index === 0} className="p-1 text-slate-400 hover:text-white disabled:opacity-30 transition-colors" title="Geser ke atas">&#8593;</button>
                        <button onClick={() => moveSliderImage(index, 1)} disabled={index === sliderBackgrounds.length - 1} className="p-1 text-slate-400 hover:text-white disabled:opacity-30 transition-colors" title="Geser ke bawah">&#8595;</button>
                      </div>
                      <button onClick={() => removeSliderImage(index)} className="ml-auto p-1 text-red-400 hover:text-red-300 transition-colors" title="Hapus gambar">
                        <span role="img" aria-label="delete">&#128465;</span>
                      </button>
                    </div>
                  ))}
                </div>
                <p className="text-muted-foreground text-xs">Gunakan tombol &#8593;&#8595; untuk mengubah urutan slide. Gambar pertama akan tampil paling awal.</p>
              </div>
            )}
            ...
            {sliderBackgrounds.length > 0 && (
              <div className="space-y-4 pt-2 border-t border-slate-700">
                ...
                <div>
                  ...
                  <input id="slideInterval" type="number" min={5} max={300} value={slideInterval} onChange={(e) => { const val = Number(e.target.value); if (val >= 5 && val <= 300) setSlideInterval(val); }} className="input mt-1 bg-slate-800/50 border-slate-700 text-white w-[200px]" />
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <button onClick={() => setAnimationType('slide_ltr')} className={`p-3 rounded-lg border transition-all ${animationType === 'slide_ltr' ? 'border-emerald-500 bg-emerald-500/10 text-white' : 'border-slate-700 bg-slate-800/50 text-muted-foreground hover:border-slate-600'}`}>
                    ...
                    <button onClick={() => setAnimationType('fade')} className={`p-3 rounded-lg border transition-all ${animationType === 'fade' ? 'border-emerald-500 bg-emerald-500/10 text-white' : 'border-slate-700 bg-slate-800/50 text-muted-foreground hover:border-slate-600'}`}>
                ...
                <div className="flex items-center justify-between p-4 rounded-lg bg-slate-800/50 border border-slate-700">
                  ...
                  <button onClick={() => setIsInfiniteLoop(!isInfiniteLoop)} className={`relative inline-flex h-7 w-12 items-center rounded-full transition-colors ${isInfiniteLoop ? 'bg-emerald-500' : 'bg-slate-600'}`}>
                ...
                <button onClick={saveBackgroundSlider} disabled={saving || sliderBackgrounds.length === 0} className="w-full flex items-center justify-center gap-2 bg-emerald-500 hover:bg-emerald-600 text-white shadow-lg shadow-emerald-500/30 px-4 py-3 text-sm font-semibold rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                  {saving ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />}
                  Simpan & Update TV
                </button>
            ...
            {sliderBackgrounds.length === 0 && (
              <button onClick={saveBackgroundSlider} disabled={true} className="w-full flex items-center justify-center gap-2 bg-slate-600 text-white shadow-lg px-4 py-3 text-sm font-semibold rounded-lg transition-colors opacity-50 cursor-not-allowed">
        ...
        <AccordionItem title="Pusat Kontrol" icon={<Zap className="w-5 h-5" />}>
          <div className="space-y-6">

            {/* Mode Muadzin Master Toggle */}
            <div className="p-4 rounded-lg bg-slate-800/50" style={{border: '1px solid rgba(16, 185, 129, 0.3)'}}>
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-white font-medium flex items-center gap-2">
                    <Zap className={`w-5 h-5 ${formState.isMuadzin ? 'text-emerald-400' : 'text-slate-400'}`} />
                    Mode Muadzin (Auto)
                  </p>
                  <p className="text-muted-foreground text-sm mt-1">Tampilkan badge "Mode Muadzin" di TV Android</p>
                </div>
                <button onClick={() => { setFormState(prev => ({ ...prev, isMuadzin: !prev.isMuadzin })); }} className={`relative inline-flex h-7 w-12 items-center rounded-full transition-colors ${formState.isMuadzin ? 'bg-emerald-500' : 'bg-slate-600'}`}>
                  <span className={`inline-block h-5 w-5 transform rounded-full shadow transition-transform ${formState.isMuadzin ? 'bg-white translate-x-6' : 'bg-white translate-x-1'}`} />
                </button>
              </div>
              <button onClick={saveAuto} disabled={saving} className="w-full flex items-center justify-center gap-2 bg-emerald-500 hover:bg-emerald-600 text-white shadow-lg shadow-emerald-500/30 px-4 py-2.5 text-sm font-semibold rounded-lg transition-colors disabled:opacity-50">
                {saving ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />}
                Simpan Mode Muadzin & Update TV
              </button>
            </div>
            ...
            {formState.isMuadzin && (
              <div className="grid grid-cols-3 gap-3">
                <button onClick={() => toggleSection('timer')} disabled={!formState.isMuadzin} className={`p-4 rounded-lg transition-all ${!formState.isMuadzin ? 'border-slate-700 bg-slate-800/50 text-muted-foreground opacity-50 cursor-not-allowed' : expandedSections.timer ? 'border-emerald-500 bg-emerald-500/10 text-white' : 'border-slate-700 bg-slate-800/50 text-white hover:border-slate-600'}`}>
                  <div className="flex flex-col items-center gap-2">
                    <Timer className="w-6 h-6" />
                    <span className="text-sm font-medium">Timer Countdown</span>
                    {expandedSections.timer ? (
                      <span className="text-xs text-emerald-400">Konfigurasi ↑</span>
                    ) : (
                      <span className="text-xs text-muted-foreground">Atur durasi adhan & iqamah</span>
                    )}
                  </div>
                </button>

                <button onClick={() => toggleSection('announcements')} disabled={!formState.isMuadzin} className={`p-4 rounded-lg transition-all ${!formState.isMuadzin ? 'border-slate-700 bg-slate-800/50 text-muted-foreground opacity-50 cursor-not-allowed' : expandedSections.announcements ? 'border-emerald-500 bg-emerald-500/10 text-white' : 'border-slate-700 bg-slate-800/50 text-white hover:border-slate-600'}`}>
                  <div className="flex flex-col items-center gap-2">
                    ...
                    {expandedSections.announcements ? (
                      ...
                      <span className="text-xs text-muted-foreground">{announcements.length > 0 ? `${announcements.length} pengumuman tersimpan` : 'Belum ada pengumuman'}</span>
                    )}
                  </div>
                </button>

                <button onClick={() => toggleSection('table')} disabled={!formState.isMuadzin} className={`p-4 rounded-lg transition-all ${!formState.isMuadzin ? 'border-slate-700 bg-slate-800/50 text-muted-foreground opacity-50 cursor-not-allowed' : expandedSections.table ? 'border-emerald-500 bg-emerald-500/10 text-white' : 'border-slate-700 bg-slate-800/50 text-white hover:border-slate-600'}`}>
                  <div className="flex flex-col items-center gap-2">
                    <Table className="w-6 h-6" />
                    <span className="text-sm font-medium">Tabel Info</span>
                    {expandedSections.table ? (
                      <span className="text-xs text-emerald-400">Konfigurasi ↑</span>
                    ) : (
                      <span className="text-xs text-muted-foreground">{tableRows.length > 0 ? `${tableRows.length} baris data` : 'Belum ada tabel'}</span>
                    )}
                  </div>
                </button>
              </div>
            ...
            {/* =============== Timer Settings Sub-Section ============== */}

            {formState.isMuadzin && (
              <div className={`border rounded-lg overflow-hidden transition-all ${expandedSections.timer ? 'border-emerald-500/30 bg-slate-800/50' : 'border-slate-700'}`}>
                <button onClick={() => toggleSection('timer')} className="w-full flex items-center justify-between p-4 text-left">
                  <span className="text-white font-medium flex items-center gap-2"><Timer className="w-5 h-5" /> Pengaturan Timer Countdown</span>
                  {expandedSections.timer ? <ChevronDown className="w-5 h-5 text-muted-foreground" /> : <div className="w-5" />}
                </button>
                ...
                {expandedSections.timer && (
                  <div className="p-4 space-y-6 border-t border-slate-700">
                    <div className="p-4 rounded-lg bg-emerald-500/10 border border-emerald-500/30 text-center">
                      <p className="text-emerald-300 font-medium">Mode Muadzin Aktif</p>
                      <p className="text-muted-foreground text-sm mt-1">Timer countdown siap dikonfigurasi untuk jadwal sholat</p>
                    </div>

                    <div className="p-4 rounded-lg bg-slate-800/50 border border-slate-700">
                      <label className="text-sm font-medium text-white block mb-2">Countdown sebelum Adhan (menit)</label>
                      <input type="number" value={timerPreAdhanMinutes} onChange={(e) => setTimerPreAdhanMinutes(Number(e.target.value))} min={1} max={30} className="w-full px-4 py-2 rounded-lg bg-slate-700 border border-slate-600 text-white focus:outline-none focus:border-emerald-500" />
                    ...
                    <div className="space-y-3">
                      ...
                      {[
                        ...
                        <div key={key} className="flex items-center gap-3">
                          ...
                          <input type="number" value={value} onChange={(e) => { if (key === 'subuh') setTimerIqamahSubuh(Number(e.target.value)); else if (key === 'dzuhur') setTimerIqamahDzuhur(Number(e.target.value)); else if (key === 'ashar') setTimerIqamahAshar(Number(e.target.value)); else if (key === 'maghrib') setTimerIqamahMaghrib(Number(e.target.value)); else if (key === 'isya') setTimerIqamahIsya(Number(e.target.value)); }} min={1} max={30} className="flex-1 px-4 py-2 rounded-lg bg-slate-700 border border-slate-600 text-white focus:outline-none focus:border-emerald-500" />
                    ...
                    <div className="p-4 rounded-lg bg-slate-800/50 border border-slate-700 space-y-3">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <button onClick={() => setTimerBeepEnabled(!timerBeepEnabled)} className={`relative inline-flex h-7 w-12 items-center rounded-full transition-colors ${timerBeepEnabled ? 'bg-emerald-500' : 'bg-slate-600'}`}>
                    ...
                    <div className="p-4 rounded-lg bg-slate-800/50 border border-slate-700">
                      <label className="text-sm font-medium text-white block mb-2">Durasi Tampilan Adhan (detik)</label>
                      <input type="number" value={adhanDisplayDuration} onChange={(e) => setAdhanDisplayDuration(Number(e.target.value))} min={1} max={60} className="w-full px-4 py-2 rounded-lg bg-slate-700 border border-slate-600 text-white focus:outline-none focus:border-emerald-500" />
                      <p className="text-muted-foreground text-xs mt-1">Berapa lama tampilan adhan akan ditampilkan di layar</p>
                    </div>
                    ...
                    <p className="text-muted-foreground text-xs text-center">
                      Timer countdown akan aktif setelah Mode Muadzin diaktifkan dan pengaturan disimpan
                    </p>
                  </div>
                )}
              </div>
            ...
            {formState.isMuadzin && (
              <div className={`border rounded-lg overflow-hidden transition-all ${expandedSections.announcements ? 'border-emerald-500/30 bg-slate-800/50' : 'border-slate-700'}`}>
                <button onClick={() => toggleSection('announcements')} className="w-full flex items-center justify-between p-4 text-left">
                  <span className="text-white font-medium flex items-center gap-2"><Bell className="w-5 h-5" /> Pengumuman & Info Jadwal</span>
                  {expandedSections.announcements ? <ChevronDown className="w-5 h-5 text-muted-foreground" /> : <div className="w-5" />}
                </button>
                ...
                {expandedSections.announcements && (
                  <div className="p-4 space-y-6 border-t border-slate-700">
                    ...
                    <p className="text-muted-foreground text-xs text-center">
                      Tambahkan pengumuman untuk jadwal kajian, Jumat, atau Hari Raya. Setiap pengumuman akan ditampilkan di TV Android.
                    </p>
                  </div>
                )}
              </div>
            ...
            {formState.isMuadzin && (
              <div className={`border rounded-lg overflow-hidden transition-all ${expandedSections.table ? 'border-emerald-500/30 bg-slate-800/50' : 'border-slate-700'}`}>
                <button onClick={() => toggleSection('table')} className="w-full flex items-center justify-between p-4 text-left">
                  <span className="text-white font-medium flex items-center gap-2"><Table className="w-5 h-5" /> Pengumuman Bentuk Tabel</span>
                  {expandedSections.table ? <ChevronDown className="w-5 h-5 text-muted-foreground" /> : <div className="w-5" />}
                </button>
                  ...
                  <div className="p-4 space-y-6 border-t border-slate-700">
                    ...
                    <div className="space-y-3 p-4 rounded-lg bg-slate-800/50 border border-slate-700">
                      ...
                      {tableHeaders.map((header, index) => (
                        <div key={index} className="flex items-center gap-2 mb-2">
                          <input type="text" value={header} onChange={(e) => updateTableHeader(index, e.target.value)} placeholder={`Kolom ${index + 1}`} className="flex-1 px-4 py-2 rounded-lg bg-slate-700 border border-slate-600 text-white focus:outline-none focus:border-emerald-500" />
                          <button onClick={() => removeTableHeader(index)} className="p-2 text-red-400 hover:text-red-300 transition-colors"><Trash2 className="w-4 h-4" /></button>
                        </div>
                      ))}
                      <button onClick={() => setTableHeaders(prev => [...prev, 'Kolom Baru'])} className="w-full py-2 text-sm bg-slate-700 hover:bg-slate-600 rounded-lg transition-colors">+ Tambah Kolom</button>
                    </div>
                    ...
                    {tableHeaders.length > 0 && (
                      <div className="space-y-4 p-4 rounded-lg bg-slate-800/50 border border-slate-700">
                        <label className="text-sm font-medium text-white block">Gaya Kolom</label>
                        {tableHeaders.map((header, index) => (
                          <div key={index} className="p-3 rounded-lg bg-slate-700/50 border border-slate-600 space-y-2 mb-2">
                            ...
                            <div className="grid grid-cols-3 gap-2">
                              <div>
                                ...
                                <input type="color" value={(columnStyles[index]?.bg_color) || '#000000'} onChange={(e) => setColumnStyles(prev => ({ ...prev, [index]: { ...prev[index], bg_color: e.target.value } }))} className="w-full h-9 px-1 py-1 rounded-lg bg-slate-700 border border-slate-600 cursor-pointer" />
                              </div>
                              <div>
                                <label className="text-xs text-muted-foreground block mb-1">Warna Teks</label>
                                <input type="color" value={(columnStyles[index]?.text_color) || '#FFFFFF'} onChange={(e) => setColumnStyles(prev => ({ ...prev, [index]: { ...prev[index], text_color: e.target.value } }))} className="w-full h-9 px-1 py-1 rounded-lg bg-slate-700 border border-slate-600 cursor-pointer" />
                              </div>
                              <div>
                                <label className="text-xs text-muted-foreground block mb-1">Perataan</label>
                                <select value={(columnStyles[index]?.align) || 'left'} onChange={(e) => setColumnStyles(prev => ({ ...prev, [index]: { ...prev[index], align: e.target.value as 'left' | 'center' | 'right' } }))} className="w-full px-2 py-1 rounded-lg bg-slate-700 border border-slate-600 text-white focus:outline-none focus:border-emerald-500">
                        ...
                        tableRows.map((row, rowIndex) => (
                          <div key={rowIndex} className="flex items-center gap-2 mb-2 flex-wrap">
                            {row.map((cell, cellIndex) => (
                              <input key={`${rowIndex}-${cellIndex}`} type="text" value={cell} onChange={(e) => updateTableCell(rowIndex, cellIndex, e.target.value)} placeholder={`Baris ${rowIndex + 1}, Kolom ${cellIndex + 1}`} className="flex-1 min-w-[200px] px-4 py-2 rounded-lg bg-slate-700 border border-slate-600 text-white focus:outline-none focus:border-emerald-500" />
                            ))}
                            <button onClick={() => removeTableRow(rowIndex)} className="p-2 text-red-400 hover:text-red-300 transition-colors"><Trash2 className="w-4 h-4" /></button>
                          </div>
                        ))
                      )}
                      <button onClick={addTableRow} className="w-full py-2 text-sm bg-slate-700 hover:bg-slate-600 rounded-lg transition-colors">+ Tambah Baris</button>
                    </div>
                      ...
                      <div className="space-y-3 p-4 rounded-lg bg-slate-800/50 border border-slate-700">
                        ...
                        {tableRows.map((row, rowIndex) => (
                          <div key={rowIndex} className="flex items-center gap-2 mb-1">
                            ...
                            <input type="color" value={rowBackgrounds[rowIndex] || '#000000'} onChange={(e) => { const newBg = [...rowBackgrounds]; newBg[rowIndex] = e.target.value; setRowBackgrounds(newBg); }} className="w-full h-9 px-1 py-1 rounded-lg bg-slate-700 border border-slate-600 cursor-pointer" />
                          </div>
                        ))}
                      </div>
                    )}
