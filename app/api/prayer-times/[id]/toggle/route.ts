...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    settings.prayer_times = settings.prayer_times || [];
    const prayerIndex = settings.prayer_times.findIndex((p: any) => p.id === id);
    if (prayerIndex >= 0) {
      settings.prayer_times[prayerIndex].status = status;
    }
