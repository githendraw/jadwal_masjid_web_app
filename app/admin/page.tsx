...
import { Loader2, Users, Building2, TrendingUp, CheckCircle, XCircle } from 'lucide-react';
...
export default function AdminPage() {
  ...
  const [stats, setStats] = useState<{ total_mosques: number; active_tenants: number; online_count: number } | null>(null);
                  ...
                  tenants.map((tenant) => (
                    <tr key={tenant.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <button
                          onClick={() => {
                            const slug = tenant.mosque_slug || localStorage.getItem('mosque_slug') || 'masjid-al-ikhlas';
                            router.push(`/settings?mosque=${slug}`);
                          }}
